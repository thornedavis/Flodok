-- portal_badges(emp_slug, emp_token)
--
-- Returns every active badge definition for the employee's org, with the
-- employee's unlock status attached. Used by the portal's dedicated Badges
-- tab so locked badges show as greyed-out placeholders alongside unlocked
-- ones — turns the tab into a collection-style "what can I earn" view
-- rather than just a history of earned items.
--
-- Sort: unlocked first, then featured definitions, then alphabetical. Manual
-- badges that have been earned multiple times collapse to one row with a
-- count and the most-recent unlock's metadata.
...
begin
  ...
  from (
    ...
    group by d.id, d.name, d.description, d.icon, d.is_featured, d.trigger_type
  ) sorted;

-- Payroll: exclude recruitment-pipeline people from payroll.
--
-- Recruits live in the same `employees` table as real employees, distinguished
-- only by `lifecycle_stage` (prospective / shortlisted / offered / signed /
-- talent_pool / no_show are recruitment stages; active / separated are actual
-- employees — same split the Employees and Performance pages use). The payroll
-- RPCs iterated over every row in `employees`, so recruits leaked into the
-- roster, the totals, and the run.
--
-- This recreates preview_payroll, payroll_trend and run_payroll to only consider
-- employees whose lifecycle_stage is in ('active','separated'). Eligibility to be
-- *paid* is unchanged: only an active contract produces a payout; employees
-- without one still appear (flagged "No active contract") at Rp 0. No signature
-- changes — purely a WHERE-clause narrowing.
...
begin
  ...
  for emp_record in
    ...
    where org_id = caller_org and lifecycle_stage in ('active', 'separated')
  loop

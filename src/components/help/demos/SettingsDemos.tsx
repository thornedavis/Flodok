...
function TabBar({ activeTab = 'Account', flashId, active }: { activeTab?: string; flashId?: string; active?: string | null }) {
  return (
    <div className="flex items-center gap-1 overflow-hidden border-b" style={{ borderColor: 'var(--color-border)' }}>
      {TABS.map((tab) => {
        ...
        return (
          <span
            ...
            className="whitespace-nowrap px-2.5 py-2 text-xs font-medium"
            style={{
...
function SectionHead({ title, children }: { title: string; children?: ReactNode }) {
  return (
    <div className="mb-4 flex items-center justify-between gap-3">
      <div className="text-base font-semibold" style={{ color: 'var(--color-text)' }}>{title}</div>
...
const ORG_STEPS: TourStep[] = [
  ...
  { target: 'org-display-name', caption: 'Display Name is an optional public-facing brand' },
...
export function SettingsOrgDemo() {
  ...
  const apply = useCallback((i: number) => {
    if (i === 1) { setEdited(true); setSaved(false) }
      ...
      setSaving(true)
      window.setTimeout(() => { setSaving(false); setSaved(true); setEdited(false) }, 600)
    } else if (i === 4) setHasLogo(true)
  }, [])
  const reset = useCallback(() => {
    setEdited(false); setSaving(false); setSaved(false); setHasLogo(false)
  }, [])
  ...
  return (
    <DesktopStage tour={tour} label="Organization settings — edit your company details and save." steps={ORG_STEPS} activeNav="Settings" url="app.flodok.com/dashboard/settings">
      <div className="p-4">
        <TabBar />
        <div className="pt-4">
          <SectionHead title="Organization Settings">
            <button
              type="button"
            ...
            <button
              type="button"
              data-demo-id="org-save-btn"
              className="rounded-lg px-3 py-1.5 text-xs font-medium text-white transition-shadow"
              style={{
                backgroundColor: 'var(--color-primary)',
                opacity: dirty ? 1 : 0.45,
                ...ringStyle(at === 'org-save-btn'),
              }}
            >
              {saving ? 'Saving…' : 'Save'}
            </button>
          ...
          <div className="mb-4 flex items-center gap-4">
            <div
              data-demo-id="org-logo-upload"
              className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-xl border text-[10px] font-medium"
              style={{
                borderColor: at === 'org-logo-upload' ? 'var(--color-primary)' : 'var(--color-border)',
                backgroundColor: hasLogo ? 'color-mix(in srgb, var(--color-primary) 12%, transparent)' : 'var(--color-bg-secondary)',
                color: hasLogo ? 'var(--color-primary)' : 'var(--color-text-tertiary)',
                ...ringStyle(at === 'org-logo-upload'),
              }}
            >
              {hasLogo ? (
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" /><circle cx="9" cy="9" r="2" /><path d="m21 15-3.5-3.5L9 20" /></svg>
              ) : (
                'Upload Logo'
              )}
            </div>
            <div className="text-xs" style={{ color: 'var(--color-text-tertiary)' }}>
              Square PNG or SVG · shown on the portal and printed documents
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field
              ...
              value={edited ? 'PT Contoh Sejahtera' : 'PT Contoh'}
              placeholder="Legal entity name"
              demoId="org-legal-name"
              active={at === 'org-legal-name'}
              caret={at === 'org-legal-name' ? <span className="ml-1 inline-block h-4 w-[2px]" style={{ backgroundColor: 'var(--color-primary)' }} /> : undefined}
            />
            <Field
              label="Display Name (optional)"
              value="Contoh"
              placeholder="Public display name"
              demoId="org-display-name"
              active={at === 'org-display-name'}
            />
            <Field label="Organization Phone" value="+62 21 5550 1234" placeholder="Phone" />
            <Field label="Company Email" value="hello@contoh.id" placeholder="Email" />
            <Field label="Industry" value="Manufacturing" placeholder="Industry" />
            <Field label="Company Size" value="25 employees" placeholder="Size" />
          </div>

          <div className="mt-3 h-4 text-xs" style={{ color: at === 'org-display-name' ? 'var(--color-text-secondary)' : 'transparent' }}>
            Optional — shown publicly on your employee portal header.
          </div>
...
function TzOption({ tz, selected, demoId, active }: { tz: TzKey; selected: boolean; demoId?: string; active?: boolean }) {
  return (
    ...
    >
      <span className="mt-0.5 flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded-full" style={{ border: `1.5px solid ${selected ? 'var(--color-primary)' : 'var(--color-border-strong)'}` }}>
...
export function SettingsTimezonesDemo() {
  ...
  return (
    <DesktopStage tour={tour} label="Timezone settings — set when 'today' rolls over for your team." steps={TZ_STEPS} activeNav="Settings" url="app.flodok.com/dashboard/settings">
      <div className="p-4">
        <TabBar />
        <div className="pt-4">
          <SectionHead title="Timezone Settings">
            <button
              type="button"
              data-demo-id="tz-cancel-btn"
              className="rounded-lg border px-3 py-1.5 text-xs font-medium transition-shadow"
          ...
          <div className="max-w-md">
            <div className="mb-1 text-[11px] font-medium" style={{ color: 'var(--color-text-tertiary)' }}>Time Zone</div>
            <div
            ...
            <div className="mt-2 text-[11px]" style={{ color: 'var(--color-text-tertiary)' }}>
              Used to decide when “today” rolls over for the manager dashboard and the daily achievement evaluation.
            </div>
            <div className="mt-2 h-4 text-xs font-medium" style={{ color: saved ? 'var(--color-success)' : 'transparent' }}>
              ✓ Saved
            </div>
          </div>
        </div>
...
const COPY = {
  id: {
    tabs: ['Akun', 'Tim', 'Integrasi', 'Payroll', 'Pencapaian', 'Persetujuan', 'Tagihan'],
...
const LANG_STEPS: TourStep[] = [
  ...
  { target: 'settings-tab', caption: 'Your language choice is remembered for next time' },
...
export function SettingsLanguageDemo() {
  ...
  return (
    <DesktopStage tour={tour} label="Language settings — switch the whole interface live." steps={LANG_STEPS} activeNav="Settings" url="app.flodok.com/dashboard/settings">
      <div className="p-4">
        {/* Localised tab bar; the Account tab carries settings-tab as a step target */}
        <div className="flex items-center gap-1 overflow-hidden border-b" style={{ borderColor: 'var(--color-border)' }}>
          {c.tabs.map((tab, idx) => {
            const on = idx === 0
            return (
              <span
                key={tab}
                data-demo-id={on ? 'settings-tab' : undefined}
                className="whitespace-nowrap px-2.5 py-2 text-xs font-medium"
                style={{
                  color: on ? 'var(--color-primary)' : 'var(--color-text-tertiary)',
                  borderBottom: on ? '2px solid var(--color-primary)' : '2px solid transparent',
                  ...(on && at === 'settings-tab' ? ringStyle(true) : {}),
                }}
              >
                {tab}
              </span>
        ...
        <div data-demo-id="lang-content" className="pt-4" style={{ borderRadius: 8, ...ringStyle(at === 'lang-content') }}>
          <div className="mb-4 text-base font-semibold" style={{ color: 'var(--color-text)' }}>{c.section}</div>

          <div className="max-w-md">
            <div className="mb-1.5 text-[11px] font-medium" style={{ color: 'var(--color-text-tertiary)' }}>{c.fieldLabel}</div>
            ...
            <div className="mt-2 text-[11px]" style={{ color: 'var(--color-text-tertiary)' }}>{c.helper}</div>

            <div className="mt-4 flex items-center gap-2 rounded-lg px-3 py-2 text-xs" style={{ backgroundColor: 'var(--color-bg-tertiary)', color: 'var(--color-text-secondary)' }}>

...
export function SlackDemo() {
  ...
  const apply = useCallback((i: number) => {
    if (i === 1) setOpen(true)
    else if (i === 2) setWs('acme-indonesia.slack.com')
    else if (i === 3) setToken('xoxb-2847-secret')
    else if (i === 4) setTested(true)
  }, [])
  const reset = useCallback(() => {
    setOpen(false)
    setWs('')
  ...
  return (
    <DesktopStage tour={tour} label="Connecting Slack — post action items from meetings to a channel." steps={SLACK_STEPS} activeNav="Settings" url="app.flodok.com/dashboard/settings?tab=integrations">
      <div className="relative p-4">
        <IntroBlock />
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0 flex-1">
              <div
                ...
                className="flex items-center gap-2"
                style={ringStyle(at === 'sl-card')}
              >
                <span className="text-sm font-semibold" style={{ color: 'var(--color-text)' }}>Slack</span>
                <StatusDot on={false} />
                <span className="text-xs" style={{ color: 'var(--color-text-tertiary)' }}>Not connected</span>
              </div>
              <div className="mt-1 text-xs" style={{ color: 'var(--color-text-tertiary)' }}>
        ...
        <DialogShell title="Slack" subtitle="Connect Slack to post action items." open={open}>
          <DlgField label="Workspace URL" value={ws} placeholder="acme-indonesia.slack.com" demoId="sl-workspace" active={at === 'sl-workspace'} />
...
const SSO_STEPS: TourStep[] = [
  { target: 'sso-card', caption: 'Single sign-on is not enabled yet' },
  { target: 'sso-enable', caption: 'Open the SSO setup' },
  { target: 'sso-metadata', caption: 'Paste your SAML metadata URL' },
  { target: 'sso-entity', caption: 'Enter your identity provider’s Entity ID' },
  { target: 'sso-domain', caption: 'Set the sign-in domain to enforce' },
  { target: 'sso-dialog-test', caption: 'Verify the connection before saving' },
]
...
export function SsoDemo() {
  ...
  const apply = useCallback((i: number) => {
    if (i === 1) setOpen(true)
    else if (i === 2) setMetadata('https://login.acmeindonesia.id/saml/metadata')
    else if (i === 3) setEntity('urn:acme:idp')
    else if (i === 4) setDomain('acmeindonesia.id')
  ...
  return (
    <DesktopStage tour={tour} label="Google SSO & SAML — let your team sign in with your identity provider." steps={SSO_STEPS} activeNav="Settings" url="app.flodok.com/dashboard/settings?tab=integrations">
      <div className="relative p-4">
        <IntroBlock />
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0 flex-1">
              <div
                ...
                className="flex items-center gap-2"
                style={ringStyle(at === 'sso-card')}
              >
                <span className="text-sm font-semibold" style={{ color: 'var(--color-text)' }}>Google SSO & SAML</span>
                <StatusDot on={false} />
                <span className="text-xs" style={{ color: 'var(--color-text-tertiary)' }}>Not connected</span>
              </div>
              <div className="mt-1 text-xs" style={{ color: 'var(--color-text-tertiary)' }}>
                Let your team sign in with Google or your SAML identity provider. Optional.
              </div>
            </div>
            <div className="shrink-0" style={{ width: 80 }}>
              <Btn demoId="sso-enable" active={at === 'sso-enable'}>Set up</Btn>
            </div>
          </div>
        </div>

        <DialogShell title="Google SSO & SAML" subtitle="Sign in with your identity provider." open={open}>
          <DlgField label="SAML metadata URL" value={metadata} placeholder="https://your-idp.com/saml/metadata" demoId="sso-metadata" active={at === 'sso-metadata'} />
          <DlgField label="IdP Entity ID" value={entity} placeholder="urn:your-org:idp" demoId="sso-entity" active={at === 'sso-entity'} />
          <DlgField label="Sign-in domain" value={domain} placeholder="yourcompany.com" demoId="sso-domain" active={at === 'sso-domain'} />
          <div className="mb-2 mt-1">
            <SuccessMsg show={tested} />
          </div>
          <div className="flex items-center justify-between gap-2">
            <SecBtn demoId="sso-dialog-test" active={at === 'sso-dialog-test'}>Test connection</SecBtn>
            <div className="flex gap-2">
              <SecBtn>Cancel</SecBtn>
              <div style={{ width: 64 }}><Btn>Save</Btn></div>
            </div>
